...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($titulo && $conteudo) {
        try {
            $stmt = $pdo->prepare("INSERT INTO comercial_mensagens (unidade_id, titulo, categoria, conteudo) VALUES (?, ?, ?, ?)");
            $stmt->execute([$unidade_id, $titulo, $categoria, $conteudo]);
            $sucesso = "Script cadastrado com sucesso!";
...
$custom_title = "Novo Script Commercial";
include 'header.php';
?>
...
<section class="dashboard-section-sq">
    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 40px; border-bottom: 2px solid #08153a; padding-bottom: 30px;">
        <div style="">
            ...
            <p style="color: var(--text-muted); font-size: var(--fs-base); margin: 5px 0 0 0; font-weight: 600; text-transform: uppercase;">Criação de template para abordagem comercial</p>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 380px; gap: 40px; align-items: start;">
                ...
                <div style="display: grid; gap: 25px;">
                    <div style="display: grid; grid-template-columns: 1fr 200px; gap: 25px;">
                        <div class="form-group-sq">
                            ...
                            <input type="text" name="titulo" required placeholder="EX: BOAS VINDAS WHATSAPP">
                        </div>
                        <div class="form-group-sq">
                            <label>Categoria</label>
                            <input type="text" name="categoria" list="categorias-padrao" placeholder="EX: VENDAS, AVISOS">
                    ...
                    <div class="form-group-sq">
                        ...
                        <textarea id="conteudo" name="conteudo" rows="12" required placeholder="ESCREVA SUA MENSAGEM AQUI UTILIZANDO AS TAGS AO LADO..." style="font-family: monospace; font-size: var(--fs-base); line-height: 1.6;"></textarea>
                    </div>

                    <div style="margin-top: 20px; padding-top: 30px; ">
                        <button type="submit" class="btn-sq" style="height: 60px; width: 100%; font-size: var(--fs-base);">CADASTRAR SCRIPT COMERCIAL</button>
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 20px;">
                    ...
                    <div style="display: grid; gap: 8px;">
                        ...
                        foreach ($tags as $tag => $label): ?>
                            <div onclick="insertTag('<?php echo $tag; ?>')" 
                                 style="cursor: pointer; display: flex; align-items: center; justify-content: space-between; background: #fff; padding: 12px 15px; border: 1px solid var(--border-color); transition: all 0.2s; hover: border-color: #08153a;"
                ...
                <div class="dashboard-container" style="padding: 30px; background: #08153a; color: #fff;">
                    <h4 style="font-size: var(--fs-xs); font-weight: 900; color: var(--primary-green); text-transform: uppercase; margin-bottom: 15px;">Dica de Alta Performance</h4>
                    <p style="font-size: var(--fs-sm); color: #94a3b8; line-height: 1.6; font-weight: 500; font-style: italic;">Mensagens personalizadas com [PRIMEIRO_NOME] possuem uma taxa de abertura 3x maior que mensagens genéricas.</p>
                </div>
            </div>
        </div>
    </form>
