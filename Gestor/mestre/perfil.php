    ...
    if ($nome && $email) {
        try {
            ...
            if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
                ...
                $novo_nome = "mestre_" . $usuario_id . "_" . time() . "." . $ext;
...
<section class="dashboard-section-sq">
    <div class="section-header-sq">
        <h2 class="section-title-sq">Meu Perfil (Mestre)</h2>
    ...
    <form method="POST" action="" enctype="multipart/form-data">
        <div style="display: grid; grid-template-columns: 300px 1fr; gap: 40px; align-items: start;">
            ...
            <div style="background: white; border: 1px solid var(--border-color); padding: 30px; text-align: center;">
                <h3 style="font-size: 11px; font-weight: 900; text-transform: uppercase; color: var(--brand); margin-bottom: 20px; letter-spacing: 1px;">Avatar Administrativo</h3>
                ...
                <div id="drop-zone" style="width: 100%; aspect-ratio: 1; background: #f8fafc; border: 2px dashed var(--border-color); display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; transition: var(--transition); position: relative; overflow: hidden;">
                    ...
                    <?php else: ?>
                        <i class="fa-solid fa-user-gear" style="font-size: 40px; color: var(--border-color); margin-bottom: 15px;"></i>
                ...
                <button type="button" onclick="document.getElementById('file-input').click()" class="btn-sq-outline" style="width: 100%; margin-top: 20px; font-size: 10px;">ALTERAR IMAGEM</button>
            </div>

            <!-- Lado Direito -->
            <div style="display: grid; gap: 30px;">
                <div style="background: white; border: 1px solid var(--border-color); padding: 40px;">
                    <h3 style="font-size: 11px; font-weight: 900; text-transform: uppercase; color: var(--brand); margin-bottom: 30px; letter-spacing: 1px; border-bottom: 1px solid #f1f5f9; padding-bottom: 15px;">Dados de Administrador</h3>
                    
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div class="form-group">
                            <label style="display: block; font-size: 11px; font-weight: 900; text-transform: uppercase; color: var(--text-muted); margin-bottom: 10px;">Nome</label>
                        ...
                        <div class="form-group">
                            <label style="display: block; font-size: 11px; font-weight: 900; text-transform: uppercase; color: var(--text-muted); margin-bottom: 10px;">E-mail</label>
                ...
                <div style="background: white; border: 1px solid var(--border-color); padding: 40px;">
                    <h3 style="font-size: 11px; font-weight: 900; text-transform: uppercase; color: var(--brand); margin-bottom: 30px; letter-spacing: 1px; border-bottom: 1px solid #f1f5f9; padding-bottom: 15px;">Segurança</h3>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div class="form-group">
                            ...
                            <input type="password" name="senha_nova" class="form-control" style="width: 100%; padding: 15px; border: 1px solid var(--border-color); font-weight: 700; background: #f8fafc;">
                        </div>
                        <div class="form-group">
                            <label style="display: block; font-size: 11px; font-weight: 900; text-transform: uppercase; color: var(--text-muted); margin-bottom: 10px;">Confirmar Senha</label>
                            <input type="password" name="senha_confirma" class="form-control" style="width: 100%; padding: 15px; border: 1px solid var(--border-color); font-weight: 700; background: #f8fafc;">
                        </div>
                    </div>
                </div>

                <button type="submit" class="btn-sq" style="padding: 20px; font-size: 13px; letter-spacing: 1px;">ATUALIZAR PERFIL MESTRE</button>
            </div>
        </div>
    </form>
</section>
...
<script>
    // Script de Drag & Drop idêntico ao da unidade
    ...
    if(dropZone) {
        ...
        dropZone.addEventListener('drop', (e) => {
            ...
            if (files.length) { fileInput.files = files; handlePreview(files[0]); }
        });
        fileInput.addEventListener('change', () => { if (fileInput.files.length) handlePreview(fileInput.files[0]); });
    }
